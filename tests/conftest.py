...
# ── Module-level state reset — replaced by pytest-isolate ──────────────────
#
# Each test now runs in a forked subprocess (see pyproject.toml: addopts
# includes --isolate), so module-level dicts/sets/ContextVars from one
# test cannot leak into the next. No manual per-module clearing needed.
#
# The skill `test-suite-cascade-diagnosis` documents the cascade patterns
# this replaces; the running example was `test_command_guards` failing 12/15
...
# ── Per-test timeout — now handled by pytest-isolate ──────────────────────
#
# Each test is forked into its own subprocess; pytest-isolate's
# ``isolate_timeout`` setting in pyproject.toml controls the timeout.
# The old SIGALRM-based _enforce_test_timeout fixture has been removed.
